function mouv_main2(s)

%
%But: géneralisation de la fonction mouv_doigts(pos,doigt) à toute la main
%     pour une fermeture complète
% différent de mouv_main par la position du pouce
...
%pouce / position codeur pouce:13823 valeur arbitraire pour
%ressembler a un poing fermé
...
for i=1:48%<--8*6 égale 6 àfois la lecture de 8 valeurs en hexa (8 valeurs par doigt)
    fread(s,1);%<-- lecture pour ne pas interferer avec les autres lectures du principal
end

%LECTEUR VITESSE  --- ADDED BY ELENA RAMPONE ---
vitesse = ones(1,6);
while(vitesse(1)~=0)&& (vitesse(2)~=0)&&(vitesse(3)~=0)&&(vitesse(4)~=0)&&(vitesse(5)~=0)&&(vitesse(6)~=0)
    for doigt = 1:6
        Doigt=['Doigt',num2str(doigt-1)];
        disp(Doigt);
        %fin_boucle=test_mouvement(doigt,s);%<--verifi si le doigt est en mouvement
        vitesse(doigt)=lecture_vitesse(doigt-1,s);
    end
end
