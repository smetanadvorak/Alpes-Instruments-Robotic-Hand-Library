...
%       integral_x = Euler(error_integral_x, (Fd(1)-Fm(1)-X(4)*cos(alpha)), 0.5);
%       integral_y = Euler(error_integral_y, (Fd(2)-Fm(2)-X(4)*sin(alpha)), 0.5);
% 
%       error = (B(1) - X(1)+Jt(1,1)*Kp(1,1)*X(4)*cos(alpha)+Jt(1,2)*Kp(2,2)*X(4)*sin(alpha)-Jt(1,1)*Ki(1,1)*integral_x-Jt(1,2)*Ki(2,2)*integral_y)^2 +...
%               (B(2) - X(2)+Jt(2,1)*Kp(1,1)*X(4)*cos(alpha)+Jt(2,2)*Kp(2,2)*X(4)*sin(alpha)-Jt(2,1)*Ki(1,1)*integral_x-Jt(2,2)*Ki(2,2)*integral_y)^2 +...
%               (B(3) - X(3)+Jt(3,1)*Kp(1,1)*X(4)*cos(alpha)+Jt(3,2)*Kp(2,2)*X(4)*sin(alpha)-Jt(3,1)*Ki(1,1)*integral_x-Jt(3,2)*Ki(2,2)*integral_y)^2;
end
